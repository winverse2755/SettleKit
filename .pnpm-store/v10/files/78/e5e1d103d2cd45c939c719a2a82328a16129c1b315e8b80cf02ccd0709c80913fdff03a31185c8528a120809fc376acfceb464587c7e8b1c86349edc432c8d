            ...
            if (_i === route.pools.length - 1) {
              ...
              if (isEthWethPool) {
                if (route.output.isNative && (_route$pools = route.pools[_i - 1]) != null && _route$pools.currency0.isNative) {
                  ...
                  _tokenAmount = CurrencyAmount.fromFractionalAmount(_pool.currency0,
                  // native currency
                  ...
                  _tokenAmount = CurrencyAmount.fromFractionalAmount(_pool.currency1,
                  // wrapped currency
                  _tokenAmount.numerator, _tokenAmount.denominator);
                }
              }
...
var V4_SWAP_ACTIONS_V2_1 = (_V4_SWAP_ACTIONS_V2_ = {}, _V4_SWAP_ACTIONS_V2_[Actions.SWAP_EXACT_IN] = [{
  name: 'swap',
  type: SWAP_EXACT_IN_STRUCT_V2_1,
  subparser: Subparser.V4SwapExactIn
}], _V4_SWAP_ACTIONS_V2_[Actions.SWAP_EXACT_OUT] = [{
  name: 'swap',
...
var V4Planner = /*#__PURE__*/function () {
  ...
  _proto.addSwapAction = function addSwapAction(type, parameters, urVersion) {
    ...
    var abiDef = urVersion === URVersion.V2_1 ? V4_SWAP_ACTIONS_V2_1[type] : V4_BASE_ACTIONS_ABI_DEFINITION[type];
    var encodedInput = defaultAbiCoder.encode(abiDef.map(function (v) {
      return v.type;
    }), parameters);
...
function createAction(action, parameters, urVersion) {
  ...
  var isSwapAction = action === Actions.SWAP_EXACT_IN || action === Actions.SWAP_EXACT_OUT;
  var abiDef = urVersion === URVersion.V2_1 && isSwapAction ? V4_SWAP_ACTIONS_V2_1[action] : V4_BASE_ACTIONS_ABI_DEFINITION[action];
  var encodedInput = defaultAbiCoder.encode(abiDef.map(function (v) {
    return v.type;
  }), parameters);
  return {
  ...
  V4BaseActionsParser.parseCalldata = function parseCalldata(calldata, urVersion) {
    ...
    return {
      actions: actionTypes.map(function (actionType, i) {
        // Use V2.1 ABI for swap actions if V2.1, otherwise use base ABI (V2.0)
        var isSwapAction = actionType === Actions.SWAP_EXACT_IN || actionType === Actions.SWAP_EXACT_OUT;
        var abiDef = urVersion === URVersion.V2_1 && isSwapAction ? V4_SWAP_ACTIONS_V2_1[actionType] : V4_BASE_ACTIONS_ABI_DEFINITION[actionType];
...
export { Actions, DYNAMIC_FEE_FLAG, Hook, HookOptions, Pool, Position, Route, Subparser, Trade, URVersion, V4BaseActionsParser, V4Planner, V4PositionManager, V4PositionPlanner, V4_BASE_ACTIONS_ABI_DEFINITION, V4_SWAP_ACTIONS_V2_1, amountWithPathCurrency, encodeRouteToPath, getPathCurrency, hookFlagIndex, priceToClosestTick, sortsBefore, tickToPrice, toAddress, toHex, tradeComparator };
